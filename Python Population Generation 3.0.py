...
for x in civ_list:
    ...
    for Timeline in range(0, Timeline):
      ...
      while t <= Timeline:
        t += 1
        population = (Ini_Pop * (math.exp(1) ** (Pop_grow * t))) - (population / (1 + (math.exp(1) * (Death_rate * t))))
        print("Current Year is ", t, "The kingdom of" , str(civ_list[0+y]), "has the total population of", format(population, ".1f"))
    civs_final[(civ_list[0 + y]),(t)] = population
    y += 1
